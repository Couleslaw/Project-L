...
namespace ProjectL.GameScene.PlayerZone
{
    ...
    public class InteractivePuzzle : MonoBehaviour, IColorPuzzleListener,
        ...
        IAIPlayerActionAnimator<PlaceTetrominoAction>
    {
        ...
        void IColorPuzzleListener.OnTetrominoPlaced(ColorImage.Color color, BinaryImage position)
        {
            // set color of specified cells
            for (int i = 0; i < _puzzleCells.Length; i++) {
                if (position[i]) {
                    _puzzleCells[i].SetFillColor(color);
                }
            }
        }

        void IColorPuzzleListener.OnTetrominoRemoved(ColorImage.Color color, BinaryImage position)
        {
        ...
        {
            _temporaryPuzzleCopy = (ColorPuzzle)_logicalPuzzle!.Clone();
        ...
        Task IAIPlayerActionAnimator<PlaceTetrominoAction>.AnimateAsync(PlaceTetrominoAction action, CancellationToken cancellationToken)
        {
            ...
            (this as IColorPuzzleListener).OnTetrominoPlaced((ColorImage.Color)action.Shape, action.Position);
