...
for x; do
	...
	magick tmp1.png -fuzz 16% -fill white -opaque black tmp2.png
	magick tmp2.png -background "rgb(0,0,0)" -alpha background -alpha off tmp3.png   # replace transparent pixels
	magick tmp3.png -compose copy -bordercolor "rgb(40,40,40)" -border 10 cards-normal/$( basename $x )  # add border
done
